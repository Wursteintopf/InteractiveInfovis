                ...
                type.map((inc, index2) => {
                  ...
                  return (
                    <g key={index2}>
                      ...
                      {index === 2 && x !== 0 && y !== 0 && zoom[0] !== zoom[1] ? <text transform={'translate(' + x + ',' + y + ')'} textAnchor='middle' style={{ fontSize: 12 }}><tspan x='0' dy='0'>{labels[index2].startsWith('5') ? labels[index2].substring(0, 10) : labels[index2]}</tspan><tspan x='0' dy='1.2em'>({Math.floor(inc.data['Erfasste Haushalte'])} households)</tspan></text> : ''}
                    </g>
                  )
                })
              }
